...
class PhoneAdvisorApp(ctk.CTk):
    ...
    def _build_sidebar(self):
        ...
        self._section_label(scroll, "1️⃣  Para que vai usar?")
        ...
        # ── Step 2: Orçamento ──────────────────────────────────────
        self._section_label(scroll, "2️⃣  Orçamento Máximo")
        ...
        # ── Step 3: Pesos da Busca ────────────────────────────────
        self._section_label(scroll, "3️⃣  Importância de cada fator")

        # Toggle to enable/disable custom weights
        toggle_row = ctk.CTkFrame(scroll, fg_color="transparent")
        toggle_row.pack(fill="x", padx=16, pady=(0, 6))
        ctk.CTkSwitch(
            toggle_row,
            text="Personalizar pesos",
        ...
        # Weight sliders frame (shown/hidden by toggle)
        self._weights_frame = ctk.CTkFrame(scroll, fg_color=COLORS["bg_surface"], corner_radius=12)
        self._weights_frame.pack(fill="x", padx=16, pady=(0, 4))
        ...
        for label, var, color in weight_defs:
            ...
            lbl = ctk.CTkLabel(row, text=label, font=("Inter", 11),
                               text_color=COLORS["text_muted"], width=76, anchor="w")
            lbl.pack(side="left")
    ...
    def _run_search(self, *_):
        ...
        hint = "⚖️ Pesos personalizados ativos." if self._use_custom_weights.get() else f"💡 {uc_info['priority_text']}"
